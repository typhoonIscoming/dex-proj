...
func (s *Scanner) scanRange(start, end uint64) error {
	infraQuery := ethereum.FilterQuery{
		FromBlock: big.NewInt(int64(start)),
		ToBlock:   big.NewInt(int64(end)),
	}

	infraQuery.Addresses = []common.Address{
		common.HexToAddress(s.Config.Contracts.PoolManager),
		common.HexToAddress(s.Config.Contracts.PositionManager),
	}

	// PoolCreated 来自 PoolManager，Transfer 来自 PositionManager
	infraQuery.Topics = [][]common.Hash{
		{SigPoolCreated, SigTransfer},
	}

	infraLogs, err := s.Client.FilterLogs(context.Background(), infraQuery)
	if err != nil {
		return err
	}

	// 仅扫描本系统池子的 Swap/Mint/Burn：
	// 1) 已在 DB/缓存中的池子；
	// 2) 当前区块范围内新创建（PoolCreated）的池子。
	poolAddrSet := make(map[common.Address]struct{}, len(s.Pools)+len(infraLogs))
	for addr := range s.Pools {
		poolAddrSet[addr] = struct{}{}
	}
	for _, infraLog := range infraLogs {
		if len(infraLog.Topics) == 0 || infraLog.Topics[0] != SigPoolCreated {
			continue
		}
		// PoolCreated 的 pool 地址是第 7 个非索引参数（data[192:224]）
		if len(infraLog.Data) >= 7*32 {
			poolAddr := common.BytesToAddress(infraLog.Data[192:224])
			if poolAddr != (common.Address{}) {
				poolAddrSet[poolAddr] = struct{}{}
			}
		}
	}

	poolAddresses := make([]common.Address, 0, len(poolAddrSet))
	for addr := range poolAddrSet {
	...
	poolQuery := ethereum.FilterQuery{
		...
		ToBlock:   big.NewInt(int64(end)),
		Topics: [][]common.Hash{
			{SigSwap, SigMint, SigBurn},
		},
	}
	if len(poolAddresses) > 0 {
		poolQuery.Addresses = poolAddresses
	}

	var poolLogs []types.Log
	if len(poolAddresses) > 0 {
		poolLogs, err = s.Client.FilterLogs(context.Background(), poolQuery)
		if err != nil {
			return err
		}
	}

	logs := append(infraLogs, poolLogs...)
	...
	log.Printf("Found %d logs in range %d-%d (infra=%d, pool=%d)", len(logs), start, end, len(infraLogs), len(poolLogs))
