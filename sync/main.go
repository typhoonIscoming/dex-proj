...
func main() {
	...
	if os.Getenv("REPAIR_POOL_STATES") == "1" {
		fmt.Println("Repairing pool states from chain...")
		if err := s.UpdateAllPoolStates(); err != nil {
			log.Fatalf("Failed to repair pool states: %v", err)
		}
		fmt.Println("Pool states repaired.")
		return
	}
