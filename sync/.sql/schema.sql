...
CREATE TABLE IF NOT EXISTS tokens (
    ...
    name TEXT,
    decimals INT,
    supports_permit BOOLEAN DEFAULT FALSE,
...
COMMENT ON COLUMN tokens.supports_permit IS '是否支持 EIP-2612 permit（通过 nonces + DOMAIN_SEPARATOR 探测）';
...
COMMENT ON COLUMN pools.pool_index IS '同一 token0/token1 交易对下的池子索引（对应合约 getPool 的 index）';
...
-- Migration: Add pool_index column to pools table
ALTER TABLE pools
ADD COLUMN IF NOT EXISTS pool_index INT DEFAULT 0;

-- Migration: Add supports_permit column to tokens table
ALTER TABLE tokens
ADD COLUMN IF NOT EXISTS supports_permit BOOLEAN DEFAULT FALSE;
...
-- 索引：便于按交易对+索引快速定位池子
CREATE INDEX IF NOT EXISTS idx_pools_token_pair_index ON pools(token0, token1, pool_index);
