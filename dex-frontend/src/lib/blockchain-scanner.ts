...
const client = createPublicClient({
	...
	transport: http(
		process.env.SEPOLIA_RPC_URL ||
			'https://sepolia.infura.io/v3/3fb192ba9f724e6497fff0a0abd04f98'
	),
});
...
export const EVENT_SIGNATURES = {
	...
	// Position NFT 事件
	IncreaseLiquidity: parseAbiItem(
		'event IncreaseLiquidity(uint256 indexed tokenId, uint128 liquidity, uint256 amount0, uint256 amount1)'
	),
	DecreaseLiquidity: parseAbiItem(
...
export class BlockchainScanner {
	...
	private async scanContract(contractAddress: string, latestBlock: bigint, connection: any) {
		...
		// 分批扫描
		const toBlock =
			fromBlock + BigInt(this.batchSize) > latestBlock
				? latestBlock
	...
	private async scanPositionManagerEvents(fromBlock: bigint, toBlock: bigint, connection: any) {
		const logs = await client.getLogs({
			...
			fromBlock,
			toBlock,
			events: [EVENT_SIGNATURES.IncreaseLiquidity, EVENT_SIGNATURES.DecreaseLiquidity],
		});
	...
	private async processSwapEvent(log: any, connection: any) {
		...
		const tokenIn = isToken0Input
			? await this.getPoolToken0(log.address)
			: await this.getPoolToken1(log.address);
		const tokenOut = isToken0Input
			? await this.getPoolToken1(log.address)
			: await this.getPoolToken0(log.address);
	...
	// 处理流动性变更事件
	private async processLiquidityChangeEvent(
		log: any,
