...
export default function SwapInterface() {
	...
	useEffect(() => {
		...
		if (fromAddr === toAddr || !isCurrentToValid) {
			const nextTo = toTokenOptions.find((token) => token.address.toLowerCase() !== fromAddr);
						...
						<div className="flex items-center space-x-2">
							<span className="text-sm text-muted-foreground">
								...
								{displayedFromBalance
									? formatTokenAmount(displayedFromBalance.formatted)
									: '0'}
							</span>
							{displayedFromBalance &&
				...
				<div className="mb-6">
					<div className="flex items-center justify-between mb-2">
						...
						<span className="text-sm text-muted-foreground">
							余额:{' '}
							{displayedToBalance
								? formatTokenAmount(displayedToBalance.formatted)
								: '0'}
						</span>
					</div>
					<div className="flex items-center justify-between p-4 bg-muted rounded-lg">
