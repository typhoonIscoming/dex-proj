...
type TransactionAction =
	'wrap0' | 'wrap1' | 'approve0' | 'approve1' | 'createPool' | 'addLiquidity';
...
export default function LiquidityManager() {
	...
	const writeMetaNodeManagerMulticall = useCallback(
		async (calldatas: `0x${string}`[]) => {
			try {
	...
	const { data: token0Data } = useReadContracts({
		contracts:
			selectedToken0Address && !isNativeTokenAddress(selectedToken0Address)
				? [
						{
	...
	const { data: token1Data } = useReadContracts({
		contracts:
			selectedToken1Address && !isNativeTokenAddress(selectedToken1Address)
				? [
						{
	...
	const detectPermitSupport = useCallback(
	...
	const refreshBalances = useCallback(async () => {
		await Promise.all([refetchToken0Balance(), refetchToken1Balance(), refetchNativeBalance()]);
	}, [refetchToken0Balance, refetchToken1Balance, refetchNativeBalance]);

	const buildPermitCalldata = useCallback(
		async ({
			tokenAddress,
			tokenName,
			value,
	...
	const TransactionStatus = () => {
		...
		const actionLabel =
			transactionAction === 'approve0' || transactionAction === 'approve1'
				? '授权'
		...
		return (
			<div className="mb-6 p-4 bg-primary/10 border border-primary/20 rounded-lg">
				...
				<div className="mt-2 text-sm text-primary">交易哈希: {shortenAddress(hash)}</div>
			</div>
		);
	};
