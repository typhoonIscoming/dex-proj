...
export default function AddLiquidity() {
	...
	const { data: token0Data } = useReadContracts({
		contracts:
			paramToken0 && !isNativeTokenAddress(paramToken0)
				? [
	...
	const { data: token1Data } = useReadContracts({
		contracts:
			paramToken1 && !isNativeTokenAddress(paramToken1)
				? [
	...
	const TransactionStatus = () => {
		...
		return (
			<div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
				...
				<div className="mt-2 text-sm text-blue-600">交易哈希: {shortenAddress(hash)}</div>
			</div>
		);
	};
			...
			<div className="bg-white rounded-2xl border shadow-lg p-4">
				...
				<div className="mb-6">
					...
					<div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
						<div className="flex items-center">
							...
							/>
							{isCalculating && (
								<Clock className="w-4 h-4 text-gray-400 animate-spin ml-2" />
							)}
						</div>
						<TokenSelector
				...
				{(isNativeTokenAddress(token0.address) || isNativeTokenAddress(token1.address)) && (
					<div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-700">
						`ETH` 代币在本页会按链上 `WETH` 授权和建池处理，不会直接用原生 ETH 做 ERC20
						multicall。
					</div>
				)}
