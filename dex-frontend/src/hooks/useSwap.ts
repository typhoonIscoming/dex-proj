...
export function useSwap() {
	...
	const getEthLikeAliases = useCallback(
		(tokenAddress: string) => {
			if (!isEthLikeToken(tokenAddress)) {
				return [tokenAddress.toLowerCase()];
			}
			return Array.from(
				new Set([TOKENS.ETH.wrappedAddress.toLowerCase(), LEGACY_WETH_ADDRESS])
			);
		},
		[isEthLikeToken]
	);
	...
	const resolveAvailableRoute = useCallback(
		async (tokenA: string, tokenB: string) => {
			...
			const matchedPools = Array.from(pools).filter((pool) => {
				...
				return (
					(aAliases.has(p0) && bAliases.has(p1)) || (aAliases.has(p1) && bAliases.has(p0))
				);
			});
			...
			const indexPath = matchedPools
				.filter((pool) => {
					...
					const p1 = pool.token1.toLowerCase();
					return (
						(p0 === tokenInResolved.toLowerCase() &&
							p1 === tokenOutResolved.toLowerCase()) ||
						(p0 === tokenOutResolved.toLowerCase() &&
							p1 === tokenInResolved.toLowerCase())
					);
				})
	...
	const estimateGasWithCap = useCallback(
		async (request: {
			address: `0x${string}`;
			...
			try {
				...
				console.warn(
					`Gas estimate failed for ${request.functionName}, fallback to ${request.fallbackGas.toString()}`,
					error
				);
	...
	const getQuote = useCallback(
		async (params: SwapParams): Promise<QuoteResult | null> => {
			...
			try {
				...
				const tokenInObj = Object.values(TOKENS).find(
				...
				if (params.tokenInDecimals == null && !tokenInObj) {
					console.debug(
						`Token ${params.tokenIn} not found in local config, fallback to 18 decimals`
					);
				}
				...
				const tokenOutObj = Object.values(TOKENS).find(
					(t) => t.address.toLowerCase() === params.tokenOut.toLowerCase()
				);
				if (!params.tokenOutDecimals && tokenOutObj)
	...
	const executeSwap = useCallback(
		async (params: SwapParams) => {
			...
			if (quote.indexPathUsed && quote.indexPathUsed.length > 0) {
				const quoteValidPath = quote.indexPathUsed.filter((index) =>
					resolvedRoute.indexPath.includes(index)
				);
				if (quoteValidPath.length > 0) {
					finalIndexPath = quoteValidPath;
				} else {
					console.warn(
						'Quote indexPathUsed is stale on-chain, fallback to available paths',
						{
				...
				const validPath = params.indexPath.filter((index) =>
					resolvedRoute.indexPath.includes(index)
				);
			...
			try {
				...
				console.warn(
					'On-chain quoteExactInput failed, fallback to API quote result',
					error
				);
			...
			if (shouldUsePermit) {
				...
				multicallData.push(
					encodeFunctionData({
						...
						args: [
							resolvedRoute.tokenIn as `0x${string}`,
							amountInWei,
							deadline,
							permitV,
							r,
