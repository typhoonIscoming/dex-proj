...
function poolsDataChainKey(poolsData: unknown): string {
	...
	return JSON.stringify(
		(poolsData as RawPoolData[]).map((p) => {
			const liq = (
				typeof p.liquidity === 'bigint' ? p.liquidity : BigInt(String(p.liquidity ?? 0))
			).toString();
			const sqrt =
				typeof p.sqrtPriceX96 === 'bigint'
					? p.sqrtPriceX96.toString()
					: String(p.sqrtPriceX96 ?? '0');
			return [
...
export const usePools = () => {
	...
	const error = poolsError != null ? '获取池子数据失败' : mapError;
