						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
							...
							<div className="bg-white border rounded-xl p-5">
								...
								<div className="text-xl font-semibold text-green-600">
									{position.totalFeesValue}
								</div>
								<div className="text-xs text-gray-500 mt-1">
									{parseFloat(position.tokensOwed0).toFixed(4)}{' '}
									{position.token0Symbol} +{' '}
									{parseFloat(position.tokensOwed1).toFixed(4)}{' '}
				...
				>
					...
					>
						...
						<div className="rounded-3xl border bg-gray-50 p-5 mb-4">
							{activeModal === 'add' ? (
								...
								<>
									...
									<div className="text-center text-8xl font-extrabold text-gray-400 mb-2">
										{selectedPercent}%
									</div>
								</>
							)}
						...
						<div className="rounded-2xl bg-gray-50 border p-4 mb-6 flex items-center justify-between">
							...
							>
								<span
									className={`h-12 w-12 rounded-full bg-white flex items-center justify-center transition-transform ${asEth ? 'translate-x-14' : 'translate-x-0'}`}
								>
						...
						<div className="mb-6 text-4xl font-bold leading-relaxed">
							<div>
								{position.token0Symbol} 头寸{' '}
								<span className="float-right">
									&lt;0.001 {position.token0Symbol}
								</span>
							</div>
							<div>
								{position.token1Symbol} 头寸{' '}
								<span className="float-right">0.100 {position.token1Symbol}</span>
							</div>
						</div>
