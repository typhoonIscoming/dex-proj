...
const WETH_ALIASES = new Set<string>(['0xfff9976782d46cc05630d1f6ebab18b2324d6b14']);
if (WETH_CANONICAL) {
	...
	try {
		if (!WETH_CANONICAL) {
			return NextResponse.json(
				{
					success: false,
					error: '缺少 WETH 地址配置（NEXT_PUBLIC_WETH9_ADDRESS / WETH9_ADDRESS）',
				},
				{ status: 500 }
			);
		}
		...
		if (!bestCandidate) {
			return NextResponse.json(
				{ error: 'Pool has no liquidity', msg: '池子没有足够的流动性' },
				{ status: 400 }
			);
		}
		...
		return NextResponse.json(
			{ error: 'Internal Server Error', msg: '内部服务器错误' },
			{ status: 500 }
		);
	}
}
