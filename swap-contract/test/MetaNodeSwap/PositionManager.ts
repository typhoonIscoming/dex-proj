...
describe("PositionManager", function () {
  async function deployFixture() {
    ...
    // 创建了一个 Token0 和 Token1 的池子，价格范围是 1 到 40000，费率是 0.3%，初始价格是 10000
  ...
  it("supports native ETH -> WETH when minting liquidity", async function () {
    ...
    await poolManager.write.createAndInitializePoolIfNecessary([
      {
        ...
        tickUpper,
        fee: 3000,
        sqrtPriceX96: BigInt(encodeSqrtRatioX96(3000, 1).toString()),
      },
    ]);
    ...
    await usdt.write.approve([manager.address, usdtAmount]);
    ...
    expect((await weth9.read.balanceOf([poolAddress])) > 0n).to.equal(true);
    expect(await manager.read.balanceOf([sender])).to.equal(1n);
  });
});
